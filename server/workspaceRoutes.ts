...
router.post("/api/workspaces", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const inviteCode = "ws-" + crypto.randomBytes(4).toString("hex");
...
router.post("/api/workspaces/join", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!workspace) {
      res.status(404).json({ error: "Invalid or expired workspace invite code." });
      return;
    }
    ...
    await recordActivity(
      ...
      "member",
      req.user!.username,
      "New member joined via invite link"
    );
...
router.post("/api/workspaces/:id/regenerate-invite", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const newCode = "ws-" + crypto.randomBytes(4).toString("hex");
    const updated = await prisma.workspace.update({
      where: { id },
...
router.delete("/api/workspaces/:id/members/:targetUserId", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await recordActivity(
      ...
      "member",
      targetUserId,
      isSelf ? "Member left workspace" : "Member removed by admin"
    );

    res.json({ success: true, message: isSelf ? "Left workspace successfully." : "Member removed." });
  } catch (err) {
...
router.post("/api/workspaces/:id/projects", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const userId = req.user!.id;
    const { id } = req.params;
    const { name, description, color, icon, start_date, due_date, priority = "MEDIUM", status = "Active" } = req.body;
...
router.patch("/api/projects/:id", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const userId = req.user!.id;
    const { id } = req.params;
    const { name, description, color, icon, start_date, due_date, priority, status } = req.body;
...
router.patch("/api/tasks/:id", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (assignee_id && assignee_id !== existingTask.assignee_id && assignee_id !== userId) {
      await createNotification(
        assignee_id,
        "Task Reassigned",
